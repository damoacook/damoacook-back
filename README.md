...
backend/
  ...
  └─ envs/   (.env.local, .env.prod)  # 예시 위치 (실제 값은 인프라 환경변수로 주입)
```

## 향후 계획

- 관리자 전용 프론트(운영자 UI)
- 공지/갤러리 리치 텍스트 CMS(하이퍼링크·글자 크기/색상/글꼴·이미지 자유 삽입)
- 시험일정 Q-NET 연동 + PDF 파싱 백업 로직(공공데이터 누락 대응)
- 이미지 최적화(WebP/AVIF 변환/썸네일), 에지 캐시
- 통계/대시보드(문의/접속/전환), 시험 커뮤니티 게시판
